...
#### 套装系统（新增）
33套职业套装，每套9件装备（头盔、胸甲、腰带、战靴、项链、戒指×2、手镯×2），分为15-75级13个等级：

| 等级 | 战士套装 | 法师套装 | 道士套装 | 加成阶段 |
|------|----------|----------|----------|----------|
| 15 | 野性套装 | 魔导套装 | - | 2/4/6/8/9件 |
| 20 | 勇气套装 | 光辉套装 | 自然套装 | 2/4/6/8/9件 |
| 25 | 守护套装 | 奥秘套装 | 野性之灵套装 | 2/4/6/8/9件 |
| 30 | 熔火套装 | 奥术套装 | 塞纳里奥套装 | 2/4/6/8/9件 |
| 35 | 烈焰套装 | 寒冰套装 | 剧毒套装 | 2/4/6/8/9件 |
| 40 | 勇气套装 | 魔能套装 | 治愈套装 | 2/4/6/8/9件 |
| 45 | 龙鳞套装 | 灵风套装 | 碎地套装 | 2/4/6/8/9件 |
| 50 | 昆虫战甲 | 昆虫法袍 | - | 2/4/6/8/9件 |
| 55 | 雷霆套装 | 虚空套装 | 暗影套装 | 2/4/6/8/9件 |
| 60 | 冰冠套装 | 暗影套装 | 巫妖套装 | 2/4/6/8/9件 |
| 65 | 风暴套装 | 熔岩套装 | 神圣套装 | 2/4/6/8/9件 |
| 70 | 奥术战铠、龙鳞重甲、凤凰战甲、怒火战铠 | 奥术秘法、凤凰法袍、怒火法袍 | - | 2/4/6/8/9件 |
| 75 | 深渊战甲、潮汐战铠、影魔战铠、日怒战铠 | 深渊魔纹、潮汐秘法、影魔法袍、日怒秘法 | - | 2/4/6/8/9件 |
...
### 套装系统
- 每套套装包含9件装备（头盔、胸甲、腰带、战靴、项链、戒指×2、手镯×2）
- 收集同一套装的装备可激活套装加成
- 2件激活第一阶段加成（基础属性）
- 4件激活第二阶段加成（进阶属性+1个特效）
- 6件激活第三阶段加成（高级属性+2个特效）
- 8件激活第四阶段加成（终极属性+3个特效）
- 9件激活完美加成（额外强力特效）
- 套装装备从Boss掉落
- 装备面板显示已激活的套装效果（X/9件）
...
### v1.6.0 (2026-01-31) - 套装系统重大升级
- ✅ **套装件数扩展**
  - 每套套装从4件扩展到9件（新增项链、戒指×2、手镯×2）
  - 共33套职业套装，覆盖15-75级13个等级
  - 新增22套套装（20/25/35/40/55/65级各3套）
  
- ✅ **套装加成阶段扩展**
  - 从2件/4件扩展到2件/4件/6件/8件/9件
  - 6件：高级属性 + 2个特效
  - 8件：终极属性 + 3个特效
  - 9件：完美加成（额外强力特效）
  
- ✅ **套装等级重新分布**
  - 从15/30/45/50/60/70扩展到15/20/25/30/35/40/45/50/55/60/65/70/75
  - 70级部分套装调整至75级（深渊、潮汐、影魔、日怒）
  
- ✅ **套装装备数据更新**
  - 为所有套装添加5件新装备（项链、戒指×2、手镯×2）
  - 共297件套装装备（33套×9件）
  - 更新掉落配置支持新装备
